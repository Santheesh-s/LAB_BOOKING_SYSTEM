...
import { Trash2, Calendar, Clock, ArrowLeft, GraduationCap, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function MyBookings() {
  ...
  const fetchBookings = async (userId) => {
    try {
      ...
      if (result.success) {
        setBookings(result.data.sort((a, b) => 
  ...
  const isRejected = (status) => status === 'rejected_by_club' || status === 'rejected_by_lab';

  const canCancelBooking = (booking) => {
    const bookingDate = new Date(booking.date);
    const now = new Date();
    return (booking.status === 'pending_club_approval' || booking.status === 'pending_lab_approval') || 
